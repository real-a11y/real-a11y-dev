...
- **`id`** — the page's **identity**: the join key for diffs, baselines and fingerprints. Defaults to the URL's path, query **and fragment** (`http://localhost:3000/pricing` → `/pricing`), so it is stable across hosts, ports and schemes — base and PR on different servers pair automatically, and renaming a page for readability changes nothing. Set it explicitly only to collapse routes the path separates, or to separate two sites that share one (two homepages both at `/` collide, and that is a hard error naming the fix rather than a silent merge).
- **`name`** — the **display label**, and nothing more. Free to change without changing what page this is. Defaults to `url`, canonicalized (`http://localhost:3000` is recorded as `http://localhost:3000/`) and stripped of userinfo and secret-looking parameters — in the query **and in the fragment**, which is where OAuth's implicit flow puts its tokens — so a credential in a URL never lands in an artifact. A fragment the redactor cannot read as parameters is truncated from the last separator before the suspect key, which keeps a hash route identifiable while printing none of what follows it.
