...
| [`@real-a11y-dev/react`](/packages/react) | React integration — `<SemanticNavigator />` component + `useSemanticTree()` / `useActiveModal()` hooks. Wraps `inspector` for inline and floating modes. | `react ^18.0.0 \|\| ^19.0.0`, `react-dom ^18.0.0 \|\| ^19.0.0` (bundles the internal `core`) |
...
| [`@real-a11y-dev/storybook-addon`](/packages/storybook-addon) | Storybook panel — preview-side extractor posts tree snapshots over the Storybook channel; manager-side React panel renders them. | `storybook` 9.x, 10.x or 11.x, `react ^18.0.0 \|\| ^19.0.0` (every entry bundles the internal `core`; the manager entry also bundles `semantic-navigator-ui`) |
