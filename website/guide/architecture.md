...
| [`@real-a11y-dev/storybook-addon`](/packages/storybook-addon) | Storybook panel — preview-side extractor posts tree snapshots over the Storybook channel; manager-side React panel renders them. | `storybook` 9.x or 10.x, `react ^18.0.0 \|\| ^19.0.0` (every entry bundles the internal `core`; the manager entry also bundles `semantic-navigator-ui`) |
