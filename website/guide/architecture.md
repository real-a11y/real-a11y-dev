...
| [`@real-a11y-dev/storybook-addon`](/packages/storybook-addon) | Storybook 8 panel — preview-side extractor posts tree snapshots over the Storybook channel; manager-side React panel renders them. | `storybook >= 8`, `react ^18.0.0 \|\| ^19.0.0` (every entry bundles the internal `core`; the manager entry also bundles `semantic-navigator-ui`) |
