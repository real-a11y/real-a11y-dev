...
> The A11y view is the same tree across every Real A11y surface — the [Chrome extension](/guide/chrome-extension), the `<SemanticNavigator />` [React component](/packages/react), the [Storybook addon](/packages/storybook-addon), the [`auditSnapshot()`](/packages/testing/snapshots#auditsnapshot-root-options) string from the testing package, the [`real-a11y tree`](/packages/cli) command in the CLI, and the [`get_semantic_tree`](/packages/mcp) tool over MCP. This page applies to all of them.
...
<p style="color: var(--vp-c-text-3); font-size: 0.9em; margin-top: -0.5rem; text-align: center;">Left: what sighted users see. Right: what assistive technology reads. Same content — different structure.</p>
