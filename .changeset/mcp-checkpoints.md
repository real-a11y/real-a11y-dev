...
- `save_checkpoint` / `diff_checkpoint` — snapshot the current page under a name, then re-snapshot and diff against it.
...
Checkpoints are in-memory, LRU-capped (20), and **survive navigation by design** — so you can `save_checkpoint("prod")`, open a preview URL, and `diff_checkpoint("prod")` for a cross-deploy accessibility diff in one session. `close_browser` clears them.
