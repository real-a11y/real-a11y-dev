...
Ordinary fragments are left exactly as they were: `#installation` and `#/dashboard/users` are useful and are not secrets, so only a fragment that actually parses as key/value pairs is rewritten, and only the keys already recognised as secret-bearing. A hash router carrying its own query (`#/cb?code=…`) is handled too — but a `?` is legal _inside_ a fragment value, so the route prefix is only honoured when it holds no `=` or `&`; otherwise an OAuth `state=/dash?tab=1` would hide the token in front of it. An untouched fragment is returned verbatim rather than re-serialized, so a plain anchor cannot pick up percent-encoding on the way through, and a bare trailing `#` survives.
