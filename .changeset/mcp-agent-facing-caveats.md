...
- **`open_page` reports headless vs headful.** Headless is the default, so a human watching for a browser window concluded it never opened. The reply now names the mode, and mentions `REAL_A11Y_MCP_HEADFUL` when there's no window to see. `buildServer` gained a `headful` option for this — the bin owns the decision, so the server can only report what it's told.
