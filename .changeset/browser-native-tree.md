...
- New API: `BrowserSession.nativeTree(): Promise<ExtractionResult>` (added to the `A11ySession` interface), backed internally by the workspace-internal `nativeTree(page)` and the pure, unit-testable `buildNativeTree(rawNodes, enrichment?, chrome?)`.
