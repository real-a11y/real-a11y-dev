...
[![npm (@real-a11y-dev/cli)](https://img.shields.io/npm/v/@real-a11y-dev/cli?label=%40real-a11y%2Fcli)](https://www.npmjs.com/package/@real-a11y-dev/cli)
...
|---------|-------------|--------------|
| [`core`](./packages/core) | Extraction engine — tree walk, role map, accessible names, queries, diff, action dispatch, DOM observer | every published package |
...
| [`browser`](./packages/browser) | Browser driver — the Playwright `BrowserSession` + injected page-bundle; the one way to extract a live Chromium's a11y tree | `cli`, `mcp`, `testing` |
| [`session-registry`](./packages/session-registry) | Session registry — named browser sessions, one command at a time per session, identity/origin pinning on reuse, idle timeout. The scheduler behind `real-a11y --session` and the MCP `session` parameter | `cli`, `mcp` |
...
Seven of the eight stay on npm at their last release and will not be updated: `@real-a11y-dev/core` and `@real-a11y-dev/browser` through `0.1.0-beta.13`, `@real-a11y-dev/serialize`, `@real-a11y-dev/audit`, and `@real-a11y-dev/snapshot` through `0.1.0-beta.12`, `ui` as `@real-a11y-dev/semantic-navigator-ui` through `0.1.0-beta.11`, and `validate` as `@real-a11y-dev/validate` through `0.1.0-beta.7`. If you imported one directly, most of the vocabulary still has a published home. For the four engine packages it is `@real-a11y-dev/testing` in every case: it re-exports `Finding`, `A11yRule`, `ALL_RULES`, `collectFindings` and the `assert*` primitives from `audit`, plus `extract`, `SerializeOptions`, `numberTabStops`, and the `auditSnapshot` / `outlineSnapshot` / `tabSequenceSnapshot` serializers from `serialize` — note the last two are the renamed `serializeOutline` / `serializeTabSequence`. The validity rules reach you through `testing`'s `toBeValidA11yTree` matcher, and the components through `inspector`, `react`, or `storybook-addon`. `core` is the largest of the eight and the one where the answer is split. Nineteen of its sixty-nine names keep a published home: `@real-a11y-dev/testing` re-exports the queries and the diff (`findByRole`, `findAllByRole`, `diffTrees`, `getOutline`, `getTabSequence`, `linearize`) with `SemanticNode`, `ExtractionResult`, `TreeDiff`, `NodeChange`, `OutlineEntry`, `RoleFilter` and `FindByRoleOptions`; `react` and `inspector` re-export the node, action and config types their own APIs name. The other fifty do not, and the two that will be missed are `extractA11yTree` and `extractDomTree` — the extractors themselves. Building published tooling directly on the engine was a supported path and is not one any more; run `real-a11y --format json` or the MCP tools instead, or use whichever package already wraps the engine for your context.

`browser` is the one whose home is elsewhere: `@real-a11y-dev/mcp` re-exports `BrowserSession` together with `A11ySession`, `BrowserSessionOptions`, `PageSnapshot` and `SnapshotOptions`, because `buildServer` takes an `A11ySession` and an embedder writing its own `SessionManager` has to name that contract — it arrives with the MCP SDK attached, so there is no longer a light install that hands you just the session. `session-registry` is the seventh, and the odd one out in the other direction: it was cut out of the CLI daemon after this rule was already settled, so it was never published at all — no last release to pin, and nobody to migrate. `@real-a11y-dev/mcp` is where the part of it an embedder must name surfaces: `SessionRegistryError` and `RegistryShutdownError` (a custom `SessionManager` signals refusals by throwing them, and the server only maps those two onto tool errors) plus the `SessionInfo` row type that `list()` returns.

A few things have no drop-in replacement. `snapshot`'s engine — the fingerprints, the `a11y-snapshot.json` artifact, the diff, the baselines — is not importable from anywhere now; the `real-a11y` CLI (`snapshot` and `diff`, both take `--format json` and `-o`) or the MCP tools is the only way to reach it. Most of `browser` is in the same position: past the five session names above, `nativeTree`, the native checkpoint helpers, `resolveTarget`, `CdpActionBackend`, `pageBundleSource`, `OpenOptions`, and the Chrome-resolution helpers (`resolveChromeExecutable`, `chromeCacheDir`, `readChromeManifest`) are reached by running a surface that carries the driver, not by importing it. `serialize`'s `foldTypography` and `SerializeInput`, and `validate`'s role-metadata helpers (`roleMeta`, `isValidRole`, `attributesForRole`, `requiredOwnedRoles`, …) are not re-exported anywhere — open an issue if you were relying on any of them. The rest of `session-registry` is run-only for the same reason, minus the disruption: `SessionRegistry` itself, `SessionLike`, `SessionFlagsLike`, `SessionRegistryOptions`, and the `SessionIdentityError` / `SessionOriginError` refusals have no published home, and never had one — you drive them through `real-a11y --session` or the MCP `session` parameter. The symbol-level version of this distinction — what carries a stability promise and what doesn't — is in [docs/STABILITY.md](./docs/STABILITY.md#public-vs-internal-api).
