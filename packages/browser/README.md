...
The **read** side is redaction-safe by construction — the producer never reads any element's live `.value`, drops the AX `value` field, and the `dom` facet copies only an allowlist of structural / accessibility attributes, so a user's field values never enter the tree. That includes what was typed into a rich-text editor (`contenteditable`, `designMode`): Chromium reports it as the editor's value *and* as the names of the nodes inside it, so below any node Chromium marks `editable` the text is withheld before normalization can promote it — a node named from that text reads `[redacted]` (not empty, which would read as unlabeled), a text-only node such as a paragraph reads unnamed, a name from the page's own markup (`aria-label`, `alt`, `title`) is kept, a node that contains an editor and is named from its contents (an `<h1>` around an inline-editable `<span>`) reads `[redacted]` too, and the `dom` facet drops `href` / `src` / `poster` / `id`, with locators anchored outside the editor. Nodes carry `a11y`, and a `dom` facet when a DOM node backs them. `buildNativeTree(rawNodes, enrichment?, chrome?)` is exported as the pure, browserless core of the producer.
