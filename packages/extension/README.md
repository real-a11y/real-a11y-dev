...
Once on, it defaults to active for the first page a **later** side-panel session connects to — one attach, no extra click — but not beyond that: a later tab switch or same-tab navigation in that same session never auto-reattaches (App.tsx's `hasAppliedNativeDefault` fires once per session, deliberately not once per tab; see its own comment for why re-checking on every tab switch would reopen the exact silent-reattach hole `hasAutoLoadedNative` exists to close).
